...
part1 <- part1 %>% mutate(FSD_ID = factor(FSD_ID))
...
happyfin %>% head
#convert YOB to number
happyfin$YOB <- as.character(happyfin$YOB) %>% as.numeric(happyfin$YOB)
```
...
happyScores  <-  as.data.frame( 
  ...
  rename(autonomy = PA1,
         ...
         productivity= PA4,
         mindfulness = PA5)
happyScores1 <- round(happyScores[,-6], 2)#round without indexing variable
happyScores1$FSD_ID <- happyScores[,6]#put indexing variable back
happyScores <- happyScores1#rounded data+indexing variable 
...
happyfin %>% select(ends_with("enter")) %>% sapply(function(x)nlevels(x))
```
